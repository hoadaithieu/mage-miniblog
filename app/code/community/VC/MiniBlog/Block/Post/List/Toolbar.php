<?php
class VC_MiniBlog_Block_Post_List_Toolbar extends Mage_Core_Block_Template
{
    /**
     * Products collection
     *
	...
	protected $_defaultPerPageValue = 10;
    ...
    {
        parent::_construct();
        $this->_orderField  = Mage::getStoreConfig('vc_miniblog/blog/default_post_sort');

		$ar = array();
		$sortingPostAr = Mage::getModel('vc_miniblog/system_config_postsort')->toOptionArray();
		if (is_array($sortingPostAr) && count($sortingPostAr) > 0) {
			foreach ($sortingPostAr as $item) {
				$ar[$item['value']] = $item['label'];
			}
		}
        $this->_availableOrder = $ar;

        switch (Mage::getStoreConfig('vc_miniblog/blog/list_mode')) {
            case 'grid':
        ...
        $this->setTemplate('vc_miniblog/post/list/toolbar.phtml');
    }
     ...
     * @return VC_MiniBlog_Block_Post_List_Toolbar
     */
    protected function _memorizeParam($param, $value)
    {
        $session = Mage::getSingleton('vc_miniblog/session');
    ...
    public function getCurrentOrder()
    {
        ...
        if ($order && isset($orders[$order])) {
            if ($order == $defaultOrder) {
                Mage::getSingleton('vc_miniblog/session')->unsSortOrder();
            } else {

                $this->_memorizeParam('sort_order', $order);
            }
        } else {
            $order = Mage::getSingleton('vc_miniblog/session')->getSortOrder();
        }
    ...
    public function getCurrentDirection()
    {
        ...
        if ($dir && in_array($dir, $directions)) {
            if ($dir == $this->_direction) {
                Mage::getSingleton('vc_miniblog/session')->unsSortDirection();
            } else {
                $this->_memorizeParam('sort_direction', $dir);
            }
        } else {
            $dir = Mage::getSingleton('vc_miniblog/session')->getSortDirection();
        }
        // validate direction
        if (!$dir || !in_array($dir, $directions)) {
            $dir = $this->_direction;
     ...
     *
     * @param string $order
     * @param VC_MiniBlog_Block_Post_List_Toolbar
     */
    ...
    public function getPagerUrl($params=array())
    {
		...
		$this->getRequest()->setAlias(Mage_Core_Model_Url_Rewrite::REWRITE_REQUEST_PATH_ALIAS, $this->helper('vc_miniblog')->getBlogUrl());
    ...
    public function getCurrentMode()
    {
        ...
        if ($mode) {
            if ($mode == $defaultMode) {
                Mage::getSingleton('vc_miniblog/session')->unsDisplayMode();
            } else {
                $this->_memorizeParam('display_mode', $mode);
            }
        } else {
            $mode = Mage::getSingleton('vc_miniblog/session')->getDisplayMode();
        }
    ...
    public function getDefaultPerPageValue()
    {
        if ($this->getCurrentMode() == 'list') {
            ...
            return $this->_defaultPerPageValue;
        }
    ...
    protected function _getAvailableLimit($mode)
    {
        ...
        $perPageConfigKey = 'vc_miniblog/blog/post_per_page';
        $perPageValues = (string)Mage::getStoreConfig($perPageConfigKey);
        $perPageValues = explode(',', $perPageValues);
        $perPageValues = array_combine($perPageValues, $perPageValues);
        if (Mage::getStoreConfigFlag('vc_miniblog/blog/allow_show_all')) {
    ...
    public function getLimit()
    {
        ...
        if ($limit && isset($limits[$limit])) {
            if ($limit == $defaultLimit) {
                Mage::getSingleton('vc_miniblog/session')->unsLimitPage();
            } else {
                $this->_memorizeParam('limit_page', $limit);
            }
        } else {
            $limit = Mage::getSingleton('vc_miniblog/session')->getLimitPage();
        }
        if (!$limit || !isset($limits[$limit])) {
            $limit = $defaultLimit;
        }
